...
public interface VPVouchersRepository extends JpaRepository<VPVouchers, Long> {
    ...
    @Query(
        value ="select * from vp_vouchers v inner join  vp_batch b  on b.id=v.batch_id  where v.product_Id='MPI_937' and v.start_date< sysdate() and v.end_date> sysdate()and v.issued_date is null and v.batch_id =b.id and b.status ='A'\n" +
